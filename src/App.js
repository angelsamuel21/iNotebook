    ...
    if (token) {
      try {
        ...
        if (response.ok) {
          ...
          setUser(null);
          showAlert("Session expired or invalid. Please login again.", "warning");
  ...
  return (
    <>
      <NoteState>
        ...
        <div className="d-flex flex-column min-vh-100"> {/* Bootstrap classes for flex column and min viewport height */}
          <Navbar user={user} handleLogout={handleUserLogout} /> {/* Pass user and new logout handler */}
          <Alert alertDetails={alert} /> {/* Pass alertDetails object */}
          {/* This div is the main content area that should grow */}
          <main className="container flex-grow-1"> {/* Bootstrap class for grow, and 'main' semantic tag */}
            <Routes>
              <Route exact path="/" element={<Home showAlert={showAlert} />} />
              <Route exact path="/about" element={<About />} />
              <Route exact path="/login" element={<Login showAlert={showAlert} fetchUserDetails={fetchUserDetails} />} /> {/* Pass showAlert and fetchUserDetails */}
              <Route exact path="/signup" element={<Signup showAlert={showAlert} fetchUserDetails={fetchUserDetails} />} /> {/* Pass showAlert and fetchUserDetails */}
              <Route exact path="/forgot-password" element={<ForgotPassword />} />  {/* Add the route */}
              <Route exact path="/profile" element={<UserProfile showAlert={showAlert} fetchUserDetails={fetchUserDetails} user={user} />} />
            </Routes>
          </main>
          <Footer /> {/* Footer will be pushed down by mt-auto because its parent is a flex column */}
        </div>
      </NoteState>
    </>
  );
